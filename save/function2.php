...
function getArrayOfDocByArrayWord($arrayWord, $page) {
  ...
  $response = $bdd->prepare(" SELECT distinct `name`, (select count(*) from position where id_document = document.id and id_word = word.id) as nombre
                              FROM `document`, `position`, `word`
                              WHERE word.word IN ($qMarks)
                              AND document.id = position.id_document
                              AND word.id = position.id_word order by nombre DESC LIMIT ".$limit_start.", ".$pagination."");
